...
#' @export
team_to_team_name <- function(value) {
  value <- fcase(
    value == "ARI", "Cardinals",
    value == "ATL", "Falcons",
    value == "BAL", "Baltimore ",
...
#' @export
table <- function(data) {
  data$team_name <- team_to_team_name(data$teamAbbreviation)
  data$team_image <- team_to_logo(data$teamAbbreviation)

  data <- data |>
    select(shortName, position, teamName, teamImage, name, salary, jdt_players_points)
