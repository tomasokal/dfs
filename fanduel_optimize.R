optimize_dk <- function(df_full, df_include, df_exclude) {
  ...
  if (dim(df_include)[1] > 0) {
    ...
    if (check_qb > 1 | check_rb > 3 | check_wr > 4 | check_te > 2 | check_dst > 1 | check_flex > 7 | sum(df_include$SALARY_FD) > 60000) {
      ...
      salary_inclusion <- sum(df_include$SALARY_FD)
